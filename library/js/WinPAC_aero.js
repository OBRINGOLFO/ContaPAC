var ImagePath   = "/library/img";
var AjaxPath    = "/library/ajax";
// ******************************************************************************* //
function rnd() {
...
(function($) {
	var AeroWindowPlugin = function(element, options) {
		...
		var obj = this;
		var settings = $.extend({
			WindowTitle: 'Inter.PAQ &#174;',
			WindowDesktopIconFile: ImagePath+'/icons/default.png',
			WindowStatus: 'window',      //maximized, minimized, window
			...
			WindowMinimize: true,
			WindowClosable: true,
			LoadiFrameContentLater: true,
			WindowDraggable: true,
			WindowAnimationSpeed: 500,
			WindowAnimation: 'easeOutCubic',
			WindowElementID: this.id,
			WindowTransparentAnimationSpeed: 300,
			WindowBorderWidth: 22,
			WindowBorderHeight: 62,
			WindowBorderMouseCursorSpacing: 25,
			TaskbarHeight: 40
		}, options || {});
		...
		$(elem).css({
			'z-index': '5000',
			width: settings.WindowOuterWidth + 'px',
			height: settings.WindowOuterHeight + 'px',
			top: settings.WindowPositionTop + 'px',
			left: settings.WindowPositionLeft + 'px',
			position: 'absolute'
		});
		$(elem).addClass('AeroWindowContainer');
		$(elem).html('<div class="AeroWindow">' + '  <table cellpadding="0" cellspacing="0" border="0">' + '    <tr>' + '      <td class="table-tl"></td>' + '      <td class="table-tm"></td>' + '      <td class="table-tr"></td>' + '    </tr>' + '    <tr>' + '      <td class="table-lm"></td>' + '      <td class="table-mm" align="right">' + '        <div class="title"><nobr><img src="' + settings.WindowDesktopIconFile + '"><span>' + settings.WindowTitle + '</span></nobr></div>' + '        <div class="buttons">' + WinBtnLeftedge + WinMinBtn + WinMaxBtn + WinRegBtn + WinCloseBtn + WinBtnRightedge + '        </div>' + '        <div class="table-mm-container" align="left">' + '          <div class="table-mm-content" style="width: ' + (settings.WindowOuterWidth - settings.WindowBorderWidth) + 'px; height: ' + (settings.WindowOuterHeight - settings.WindowBorderHeight) + 'px;">' + SourceContent + '          </div>' + '        </div>' + '      </td>' + '      <td class="table-rm"></td>' + '    </tr>' + '    <tr>' + '      <td class="table-bl"></td>' + '      <td class="table-bm"></td>' + '      <td class="table-br"></td>' + '    </tr>' + '  </table>' + '</div>');
		...
		if (WindowContainer.find('iframe').length > 0) {
			...
			WindowContainer.find('iframe').attr({
				src: $(elem).find('iframe').attr('src')
			});
			...
			switch (NewSize) {
				...
				case 'minimize':
					...
					WindowContainer.draggable({
						cursorAt: {
							cursor: "crosshair",
							top: settings.WindowBorderMouseCursorSpacing,
							left: (settings.WindowOuterWidth / 2)
						}
					});
	...
	$.fn.AeroWindowLink = function(options) {
		...
		return this.each(function() {
			...
			var id = "window-" + rnd();
			$('body').append('<div id="' + id + '" style="display: none;">' + '<iframe  src="" width="100%" height="100%" style="display:none; overflow: hidden; border: 0px;" frameborder="0" id="' + id + '-iframe"></iframe>' + '</div>');
...
$(document).ready(function() {
	...
	function GenerateNewWindow(NewWindowURL, WindowType, ShowDesktopIcon) {
		...
		if (WindowType != 'image') {
			if (NewWindowURL.substr(0, 10) == 'http://www') {} else if (NewWindowURL.substr(0, 3) == 'www') {
		...
		if (NewWindowURL == 'http://www.google.com') {
			NewWindowURL = 'http://www.google.com/';
		}
		if (WindowType == 'image') {
			...
			$('body').append('<div id="' + Randomnumber + '">' + '  <iframe src="' + NewWindowURL + '" scrolling="no" width="100%" height="100%" style="overflow: hidden; border: 0px;" frameborder="0"></iframe>' + '</div>');
			WindowOuterWidth = 800;
			WindowOuterHeight = 500;
		}
		if (NewWindowURL == 'http://www.google.com/') {
		...
		image = ImagePath+'/icons/default.png';
    ...
    window.GenerateNewWindowLightbox = function(NewWindowURL, WindowTitle, ShowDesktopIcon, e) {
		...
		StartWindowOuterWidth = 200;
		StartWindowOuterHeight = 100;
		EndWindowOuterWidth = 1024; //ancho
		EndWindowOuterHeight = 540; //alto
		MinSpacing = 20;
		if (NewWindowURL == null) {
			NewWindowURL = prompt("URL for the new Window", "http://www.google.com");
		}
		...
		if (WindowTitle == null) WindowTitle = "WinPAC &#174; ";
		...
		if (WindowType == 'Image') {
			...
			$(icon).load(function() {
				...
				if (NewWindowURL == 'http://www.google.com/') {
					image = ImagePath+'/icons/google.png';
				} else {
					image = ImagePath+'/logo-icon.png';
				}
				ShowWindow();
		...
		function DropContent() {
			if (WindowType == 'Link') {
				window.setTimeout(function() {
					$('#' + Randomnumber).find('.table-mm-content').html('<iframe src="' + NewWindowURL + '" scrolling="no" width="100%" height="100%" style="overflow: hidden; border: 0px;" frameborder="0"></iframe><div class="iframeHelper"></div>');
					$('#' + Randomnumber).find('.iframeHelper').css({
						'display': 'none'
					});
					if (!$.browser.msie) {
	...
	$('#AeroWindowLinks').find('a').click(function(e) {
		GenerateNewWindowLightbox($(this).attr('href'), $(this).attr('title'), null, false, e);
		return (false);
	});
    /** ******************************* **/    
	...
	if ($('#Taskbar').length == 0) {
		...
		$('#Taskbar').append('<div style="float: left;" id="Start-Menu-Button"  title="Menú de Inicio"> </div>');
		$('#Taskbar').append('<div id="TaskbarPoints"> </div>');
		$('body').append('<div id="Start-Menu"> </div>');
		$('#Taskbar').append('<div title="Mostrar Escritorio" id="TaskbarShowDesktop"></div>');
        $('#Taskbar').append('<div id="TaskbarTime" title="Tiempo del sistema."></div>');
        $('#Taskbar').append('<div id="TaskbarPoints" style="float: right;"></div>');
	}
    /** ******************************* **/    
	if (ShowTaskbar == true) {
    ...
    /** *******************************     
	$.get(AjaxPath + '/StartmenuContent.php', function(data) {
		$('#Start-Menu').html(data);
		$('#Start-Menu').load(AjaxPath + '/StartmenuContent.php', function() {
